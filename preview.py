...
def visualize_outputs(input_dir='outputs', output_dir='visualization'):
    ...
    Each plot title is formatted as "[category] (file name) by [model name]", and
    the y-axis is positioned with 0 as the lower bound to represent ground level.
    """
    ...
    for model_dir in input_path.iterdir():
        ...
        for category_dir in model_dir.iterdir():
            ...
            for json_file in category_dir.glob('*.json'):
                ...
                # Set x and z limits centered on data
                ax.set_xlim(mid_x - half_span, mid_x + half_span)
                ax.set_zlim(mid_z - half_span, mid_z + half_span)
                # For y-axis, ground level at 0
                y_lower = 0
                y_upper = max(max_y, mid_y + half_span)
                ...
                # Title formatted as [category] (filename) by [model]
                filename = json_file.stem
                title = f"{category} ({filename}) by {model}"
                ax.set_title(title)

                # Save the figure
                out_file = save_dir / f"{filename}.png"
                try:
